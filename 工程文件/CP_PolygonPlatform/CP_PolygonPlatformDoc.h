...
public:
    ...
    // 下面变量将被保存
    CP_Polygon m_a, m_b;
	vector<Segment> ABEdge;//定义AboolB结果中边集合
 ...
 // //////////////////////////////////////////////////////////////
    ...
    int  m_flagSelectPolygon, m_flagSelectRegion, m_flagSelectID;// 定位拾取的内容,m_flagSelectPolygon(0: A; 1: B)
    ...
    VT_PointArray m_flagAddPointArray;//记录临时点集

	bool m_flagShowA, m_flagShowB, m_flagShowPointID, m_flagShowResult;//显示A、B、点标志、布尔运算结果 true: 显示; false: 不显示。

    bool m_flagMoveSame; // true: 移动重合的点; false: 重合点分开移动。
    bool m_flagShowTriangleFace;  //剖分面 true: 显示; false: 不显示。
